...
namespace GameEngine
{
    class MyMath
    {
        ...
        public static double PerlinNoise(Vector inVector, Vector frequency, int iterations)
        {
            ...
            Vector myVector = inVector + new Vector(12345, 456789, 321);
            if (iterations == 0) iterations = 1;//devide by zero safety

            for (int i = 0; i < iterations; i++)
            {
                double X = myVector.X * i * frequency.X;
                double Y = myVector.Y * i * frequency.Y;
                double Z = myVector.Z * i * frequency.Z;
