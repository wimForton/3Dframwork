...
namespace GameEngine
{
    class RenderOpenGlTemplate : RenderOpenGL
    {
        ...
        public RenderOpenGlTemplate(List<IRenderableGeo> inGeoList, float inFps, int initialWindowWidth, int initialWindowHeight, string initialWindowTitle) : base(initialWindowWidth, initialWindowHeight, initialWindowTitle)
        {
            ...
            fps = inFps;
            sceneRotation = Matrix4x4.CreateFromYawPitchRoll(sceneRotationY, sceneRotationX, 0.0f);
            scenePosition = Matrix4x4.CreateTranslation(0.0f, 0.0f, -15.0f);
            if (myRendergeo.Count > 0)
            {
                IterateGeoTree.LayoutNodes(myRendergeo);
                for (int i = 0; i < myRendergeo.Count; i++)
                {
                    myRendergeo[0].MakeVaoArray();
                }
            }
            Wimapp3D.MainWindow.AppWindow.TimeSliderStack.Children.Add(timeSlider);
        }
        ...
        protected unsafe override void Update()
        {
            if (myRendergeo.Count > 0)
            {
                IterateGeoTree.ParentChildIterate(myRendergeo);
                ...
                for (int i = 0; i < myRendergeo.Count; i++)
                {
                    if (myRendergeo[i].GetVaoArray() != null)
                    {
