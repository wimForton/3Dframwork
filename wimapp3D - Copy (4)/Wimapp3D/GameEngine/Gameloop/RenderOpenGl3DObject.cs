    ...
    class RenderOpenGl3DObject : RenderOpenGL
    {
        ...
        public RenderOpenGl3DObject(List<IRenderableGeo> inGeoList, float inFps, int initialWindowWidth, int initialWindowHeight, string initialWindowTitle) : base(initialWindowWidth, initialWindowHeight, initialWindowTitle)
        {
            ...
            for (int i = 0; i < myGame.MyPlayers.Count; i++)
            {
                ...
                myRendergeo[i].Position = myPlayerPath.Points[pathPointNum];
            ...
            for (int i = 0; i < myRendergeo.Count; i++)
            {
                ...
                myVaoList.AddRange(myRendergeo[i].GetVaoArray());
            }
        ...
        private void UpdateGame()
        {
            for (int i = 0; i < myGame.MyPlayers.Count; i++)
            {
                ...
                if (KeyframeCount > 1)
                {
                    ...
                    Vector keyPos1 = myPlayerPath.Points[(int)Math.Floor(tweenPathpointNum)];
                    Vector keyPos2 = myPlayerPath.Points[(int)Math.Ceiling(tweenPathpointNum)];
            ...
            {
                myRendergeo[i].Update();
                myVaoList.AddRange(myRendergeo[i].MakeVaoList());
                //myVaoList.AddRange(ObjectToOpengl.MakeVaoList(myRendergeo[i]));
