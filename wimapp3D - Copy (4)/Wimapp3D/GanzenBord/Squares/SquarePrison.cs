...
{
    class SquarePrison : Square, iSquare
    {
        public override string Name { get; set; } = "Prison";
        public override Vector Color { get; set; } = Vector.setNew(1, 0.5, 0);
        public SquarePrison(int pos) : base(pos)
        {
            SquarePos = pos;
        }
        public override void Actions(Player inPlayer, Game inGame)
        {
            if (inPlayer.turnsWaitedAtThisSquare < 3)
            {
                inGame.GameOutput($"Player has arrived at {Name} square", Vector.setNew(0, 1, 1));
                inGame.GameOutput($"I'm stuck here for {3 - inPlayer.turnsWaitedAtThisSquare} more turns", Vector.setNew(0, 1, 1));
